...
public interface StatRepository extends JpaRepository<StatSvc, Integer> {
    ...
    @Query("select new ru.practicum.dto.dto.ViewStats(st.app, st.uri, count(distinct st.ip) as hits) " +
            "from StatSvc as st " +
    ...
    @Query("select new ru.practicum.dto.dto.ViewStats(st.app, st.uri, count(st.ip) as hits) " +
            "from StatSvc as st " +
            "where st.timestamp between :start and :end " +
            "group by st.app, st.uri ")
