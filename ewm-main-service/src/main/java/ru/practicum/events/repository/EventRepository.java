...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    List<Event> findAllPublishStateOnlyAvailable(EventStatus state, LocalDateTime rangeStart, List<Long> categories,
    ...
    @Query("select e " +
            ...
            "and (:text is null or (upper(e.annotation) like upper(concat('%', :text, '%'))) " +
            "or (upper(e.description) like upper(concat('%', :text, '%'))))")
    List<Event> findAllPublishStateOnlyNotAvailable(EventStatus state, LocalDateTime rangeStart, List<Long> categories,
                                                    Boolean paid, String text, PaginationSetup pageable);
